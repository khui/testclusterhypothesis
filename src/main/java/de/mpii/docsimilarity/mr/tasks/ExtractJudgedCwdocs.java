...
import de.mpii.docsimilarity.mr.input.clueweb.FilteredCw09InputFormat;
import de.mpii.docsimilarity.mr.input.clueweb.ClueWebWarcRecord;
import de.mpii.docsimilarity.mr.input.clueweb.FilteredCw12InputFormat;
 ...
 * @author khui
 */
public class ExtractJudgedCwdocs extends Configured implements Tool {

    private static final Logger logger = Logger.getLogger(ExtractJudgedCwdocs.class);

    private static class Constants {

        static String SEP = "/";

        static String OUTDIRNAME = "Extracted";

    }

    private static enum Counter {

        TOTAL, PAGES, ERRORS, SKIPPEDPAGES, SKIPPEDTERMS, DFFILTERED, QIDPAGES
    };
    ...
    public int run(String[] args) throws Exception {
         ...
         */
        Job job = new Job(getConf(), ExtractJudgedCwdocs.class.getSimpleName() + " Extract doc for qrels: " + colname);
        job.setJarByClass(ExtractJudgedCwdocs.class);
        job.setNumReduceTasks(16);
        //job.setUserClassesTakesPrecedence(true);

        DistributedCache.addCacheFile(new Path("/user/khui/data/cwterms/tfidfdocvector/baseline.cwidsqid." + colname).toUri(), job.getConfiguration());
        job.getConfiguration().set(GlobalConstants.CWIDS_FILE_NAME_PREFIX, "baseline.cwidsqid." + colname);
    ...
    public static void main(String[] args) throws Exception {
        int exitCode = ToolRunner.run(new ExtractJudgedCwdocs(), args);
