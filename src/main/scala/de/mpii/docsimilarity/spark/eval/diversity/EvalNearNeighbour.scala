...
/**
  ...
  */
object EvalNearNeighbour {
  var docvecdir:String = ""
  var outdir = ""
  var qreldir = ""
  var parNum = 1024
  var expname:String = "vectorize doc"
  ...
  def main(args : Array[String]): Unit = {
    ...
    val conf: SparkConf = new SparkConf()
      .setAppName("Eval on near-neighour benchmark (diversity) " + expname)
    ...
    val qidCwidLabel: Map[Int, Map[String, Set[Int]]] = readDiversityQrel(sc,qreldir + "/qrels.diversity.wt*", parNum)

    val validQids = qidCwidLabel.keySet
    ...
    validQids
      ...
      .foreach(
        qid => {
          ...
          val cwidTopSimilarDocs: RDD[(String, Array[(String, Double)])] = topkSimilarDocs(sc, qid, docvec2eval, bcqrel.value(qid), topks.max, parNum)
  ...
  /**
    ...
    * @return
    */
  private def topkSimilarDocs(sc : SparkContext, qid: Int, docvectorfile : String, cwidLabel: Map[String, Set[Int]], maxTopK: Int=50, parNum:Int=1024) :  RDD[(String, Array[(String, Double)])] = {
    ...
    val cwidSortedCwids: RDD[(String, Array[(String, Double)])] = cwidVector
      ...
      .map{case(x,y) =>((x._1._1,y._1._1), cosineSimilarity(x._1._2, y._1._2))}
  ...
  /**
    ...
    * @return
    */
  private def simlarityEval (topkSimilarDocs : RDD[(String, Array[(String, Double)])], cwidLabel: Map[String, Set[Int]], topks : Array[Int]): Map[Int, Double]= {
    topkSimilarDocs
      .flatMap { case (doc, docSimi) =>
        val subtopics: Set[Int] = cwidLabel(doc)
       // val subtopicKRel: Array[Array[(Int, Int, Double)]] =
        subtopics
          .flatMap {
            subtopic =>
                val labels: IndexedSeq[Int] =
                  (0 until Math.min(topks.max, docSimi.length))
                    .map(idx => if (cwidLabel(docSimi(idx)._1).toSet.contains(subtopic)) 1 else 0)
                val subtopicKRel: Array[((Int, Int), Double)] =
                  topks
                    .map(topk => (subtopic, topk) -> labels.slice(0, topk).sum / topk.toDouble)
              subtopicKRel
          }

      }
      // aggregate on (subtopic, topk) pairs
      .aggregateByKey[Array[Double]](Array.empty[Double])((a, t) => a.+:(t), (a1, a2) => a1.++(a2))
      // average for each (subtopic, topk) pair
      .map(t2 => t2._1._2 -> t2._2.sum / t2._2.length.toDouble)
      // aggregate on topk
      .aggregateByKey[Array[Double]](Array.empty[Double])((a, t) => a.+:(t), (a1, a2) => a1.++(a2))
      // average among different subtopics, computing macro-avg
  ...
  def readDiversityQrel(sc : SparkContext, qrelf: String, parNumber: Int=128): Map[Int, Map[String, Set[Int]]]={
    val addPair2Map = (m :Map[String, Set[Int]], p :(String, Int)) =>
    {
      val nv = m.getOrElse(p._1, Set.empty[Int]).+(p._2)
      m.updated(p._1, nv)
    }
    val mergeMaps = (m1 :Map[String, Set[Int]],m2 :Map[String, Set[Int]]) =>
    {
      val cwids: Set[String] = m1.keySet ++ m2.keySet
      cwids
        .map(cwid => cwid -> m1.getOrElse(cwid, Set.empty[Int]).++(m2.getOrElse(cwid, Set.empty[Int])))
        .toMap
    }
    ...
    sc
      ...
      .map{cols => cols(0).toInt -> (cols(2) -> cols(1).toInt)}
      .aggregateByKey[Map[String,Set[Int]]](Map.empty[String,Set[Int]])(addPair2Map, mergeMaps)
      // only consider query with more than one subtopics
      .filter(qidCwidSubtopics => qidCwidSubtopics._2.flatMap(m => m._2).toSet.size > 1)
      .collectAsMap()
      .toMap
  }
