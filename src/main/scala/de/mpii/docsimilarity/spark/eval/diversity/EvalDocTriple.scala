...
object EvalDocTriple {
  ...
  def main(args : Array[String]): Unit = {
    ...
    val conf: SparkConf = new SparkConf()
      .setAppName("Eval on doctriple similarity benchmark (diversity) " + expname)
      ...
      pool.execute(
        new Runnable{
          override def run(): Unit ={
            ...
            val tripleexists = fs.exists(new Path(doctripledir + "/" + qid))
            if (docvecdir.length > 0 && tripleexists) {
